/*=========================================================================
  ...
  Module:    $RCSfile: itkDiffusionTensor3DReconstructionNonlinearImageFilter.h,v $
  Language:  C++
...
public:
  ExponentialLeastSquaresFunction(unsigned int                   number_of_residuals,
                                  ...
                                  const vnl_vector<TSignalType>& signal,
                                  TSignalType                    s0): 
    vnl_least_squares_function(6,number_of_residuals,no_gradient), 
    m_Design_Matrix(design_matrix),
    m_S0(s0)
  {
    m_Signal.set_size(number_of_residuals);
    std::copy(signal.begin(),signal.end(),m_Signal.begin());
  }
  
  void f(const vnl_vector<double> &x,
         vnl_vector<double> &fx)
  {
    vnl_vector<double> attenuation(n_);
    attenuation = - m_Design_Matrix * x;
//     std::transform(attenuation.begin(),attenuation.end(),
//                    attenuation.begin(),
//                    std::exp);
    for(unsigned int i = 0; i < n_; ++i)
      attenuation[i] = exp(attenuation[i]);
    
    fx = m_Signal - m_S0*attenuation;
  }
...
public:
  ...
  itkTypeMacro(DiffusionTensor3DReconstructionNonlinearImageFilter, 
                                                      ImageToImageFilter);
...
//  void AddGradientImage( const GradientDirectionType &, const GradientImageType *image);
  ...
  itkSetMacro( Step, TTensorType );
  itkGetMacro( Step, TTensorType );
