...
template <class RealType>
void generateSHBasisMatrix(int numterms, int numgradients, vnl_matrix<RealType> & sgradients,
                           vnl_matrix<RealType> & basismat);

template <class RealType>
void generateSHBasisMatrixPseudoInverse( vnl_matrix<RealType>& cart_gradients, const RealType lambda,
                                         const unsigned int num_terms, const unsigned int order,
                                         vnl_matrix<RealType>& sh_basis_mat_pi );

template <class RealType>
void generateWeightedSHBasisMatrixPseudoInverse( vnl_matrix<RealType>& cart_gradients,
                                                 const vnl_diag_matrix<RealType>& W, const RealType lambda,
                                                 const unsigned int num_terms, const unsigned int order,
                                                 vnl_matrix<RealType>& sh_basis_mat_pi );

template <class RealType>
void generateWeightedSHBasisMatrixPseudoInversePrecomputed( const vnl_matrix<RealType> & B,
                                                            const vnl_diag_matrix<RealType> & L,
                                                            const vnl_diag_matrix<RealType>& W, const RealType lambda,
                                                            vnl_matrix<RealType>& sh_basis_mat_pi );

template <class RealType>
void generateSHBasisMatrixPseudoInversePrecomputed( const vnl_matrix<RealType> & B, const vnl_diag_matrix<RealType> & L,
                                                    const RealType lambda, vnl_matrix<RealType>& sh_basis_mat_pi );

template <class RealType>
void computeSHOrigBasisMat( vnl_matrix<RealType>& sh_basis_mat, vnl_matrix<RealType>& cart_gradients,
                            const unsigned int num_terms );

template <class RealType>
void computeSHRegularizationMatrix( vnl_diag_matrix<RealType> & regdiag, const unsigned int num_terms,
                                    const unsigned int order );

template <class RealType>
void computeSHHuberWeightMatrix( vnl_diag_matrix<RealType> & W, const unsigned int numoriggvectors,
...
template <class RealType>
RealType huberWeightFcn( RealType scaledResidual, RealType C, bool & isOutlier );
