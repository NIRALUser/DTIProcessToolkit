      DOUBLE PRECISION FUNCTION XXZABS(ZR, ZI)
C***BEGIN PROLOGUE  XXZABS
C***REFER TO  ZBESH,ZBESI,ZBESJ,ZBESK,ZBESY,ZAIRY,ZBIRY
C
C     XXZABS COMPUTES THE ABSOLUTE VALUE OR MAGNITUDE OF A DOUBLE
C     PRECISION COMPLEX VARIABLE CMPLX(ZR,ZI)
C
C***ROUTINES CALLED  (NONE)
C***END PROLOGUE  XXZABS
...
C-----------------------------------------------------------------------
      ...
      IF (U.GT.V) GO TO 10
      Q = U/V
      XXZABS = V*DSQRT(1.D+0+Q*Q)
      RETURN
   10 Q = V/U
      XXZABS = U*DSQRT(1.D+0+Q*Q)
      RETURN
   20 XXZABS = 0.0D+0
      RETURN
      END
