#ifndef _itkTensorWithFaFileWriter_txx
#define _itkTensorWithFaFileWriter_txx
#include "itkTensorWithFaFileWriter.h"
...
template <typename T>
void
TensorWithFaFileWriter<T>::GenerateData()
{
  ...
  outstr << std::scientific << std::setprecision(17);
  ...
  outstr << "SCALARS scalar float 1" << std::endl;
  outstr << "LOOKUP_TABLE default" << std::endl;
  typedef ImageRegionConstIterator<InputImageType> IteratorType;
  IteratorType it(in, inregion);
  for( it.GoToBegin(); !it.IsAtEnd(); ++it )
    {
    float writedata = it.Get().GetFractionalAnisotropy();
    ByteSwapper<float>::SwapRangeFromSystemToBigEndian(&writedata, 1);
    outstr.write(reinterpret_cast<const char *>(&writedata), sizeof(float) );
    }

  outstr << std::endl << "TENSORS tensor float" << std::endl;
  for( it.GoToBegin(); !it.IsAtEnd(); ++it )
    {
