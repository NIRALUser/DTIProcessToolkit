...
public:
  void initialize( typename TTensorEstimateType::Pointer testim)
  {
    ...
    typedef itk::Functor::TensorApparentDiffusionCoefficient<itk::DiffusionTensor3D<double>,
                                                             vnl_vector_fixed<double, 3>, double> ADC;
    while( !git.IsAtEnd() )
      {
      ...
      for( unsigned int i = 1; i < numberOfGradientImages; ++i )
        {
        dwiVector[i] =
          static_cast<short int>(dwiVector[0]
                                 * exp(-bvalue * ADC() (m_TrueTensor, gradientDirections->ElementAt(i) ) ) );
        }
      git.Set( dwiVector );
      ++git;
      }
